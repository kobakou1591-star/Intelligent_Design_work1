...
# 波長の設定 (1549.5nm ~ 1550.5nm, 100ポイント -> 結合して200次元) [6]
wl = torch.linspace(1549.5e-9, 1550.5e-9, 100)

##### 2. データセットの準備（tau1, tau2を固定） [1]
# tau1_range と tau2_range を同一値 [0.9, 0.9] に設定して固定します
dataset = MRRDataset(
    num_samples=1000,
    tau1_range=[0.9, 0.9],
    tau2_range=[0.9, 0.9],
...
# スペクトル (Input) のスケーリング (Tensorが返る) [7]
...
##### 5. モデル、損失関数、最適化手法の定義 [9, 10]
# 入力: スペクトル次元, 出力: パラメータ次元(3次元)
...
plt.yscale('log')
plt.title("Inverse Problem Training History (Fixed tau1, tau2)")
plt.xlabel("Epoch")
plt.ylabel("MSE Loss")
plt.legend()
...
# 3つのパラメータ (tau1, tau2, alpha) の散布図プロット
fig, axes = plt.subplots(1, 3, figsize=(18, 5))
param_names = [r'$\tau_1$ (Fixed: 0.9)', r'$\tau_2$ (Fixed: 0.9)', r'$\alpha$']
